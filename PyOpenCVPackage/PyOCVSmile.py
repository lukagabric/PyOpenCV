...
def detect_and_draw(image):
    ...
    # convert color input image to rgb
    cvImage = cv.CreateImage(image_size, 8, 1)
    cv.CvtColor(image, cvImage, cv.CV_BGR2GRAY)
    ...
    haarlocation = '/Users/lukagabric/Workspaces/Kepler/Workspaces/PyOpenCV/smile.xml' # Mac OS X example
    
    # load haar cascade data
    cascade = cv.Load(haarlocation)
    
    smiles = cv.HaarDetectObjects(cvImage, cascade, storage, 1.1, 3, cv.CV_HAAR_DO_CANNY_PRUNING)
 
    if smiles:
        print 'smiles detected!    Press Escape to exit program.'
        for i in smiles:
